...
describe('Task e2e test', () => {
  ...
  const taskSample = { title: 'furlough thorn', status: 'PENDING', priority: 'HIGH' };
  ...
  describe('new Task page', () => {
    ...
    it('should create an instance of Task', () => {
      cy.get(`[data-cy="title"]`).type('oof tightly');
      cy.get(`[data-cy="title"]`).should('have.value', 'oof tightly');

      cy.get(`[data-cy="description"]`).type('spirit now');
      cy.get(`[data-cy="description"]`).should('have.value', 'spirit now');

      cy.get(`[data-cy="dueDate"]`).type('2024-12-19');
      cy.get(`[data-cy="dueDate"]`).blur();
      cy.get(`[data-cy="dueDate"]`).should('have.value', '2024-12-19');
