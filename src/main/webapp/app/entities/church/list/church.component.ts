...
export class ChurchComponent implements OnInit {
  ...
  trackId = (item: IChurch): number => this.churchService.getChurchIdentifier(item);
  ...
  delete(church: IChurch): void {
    const modalRef = this.modalService.open(ChurchDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.church = church;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.churches = this.refineData(dataFromBody);
  }

  protected refineData(data: IChurch[]): IChurch[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IChurch[] | null): IChurch[] {
    return data ?? [];
  }
